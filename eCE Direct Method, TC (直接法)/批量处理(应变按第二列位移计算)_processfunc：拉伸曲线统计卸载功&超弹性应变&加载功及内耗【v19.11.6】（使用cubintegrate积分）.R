process.func = function(sample.name, #样品名
                        file.no = 1,#待处理文件序号
                        num_cycle = 2,#循环数
  ...
  find.index.end = function(data, index_current,stress_loading,stress_unloading) {
    for (i in index_current+9:nrow(data)){
      ...
      #寻找点：此点前第30个数超过卸载应力+5，且此点小于卸载应力-2，且其相邻20个点至少17个大于该点
  ...
  #fname = file.choose()
  fname = paste(sample.name,"_",file.no,".csv", sep = "")
  data = read.csv(fname, )
  names(data)[4:5] <- c("Stress","Strain")#为特定列命名
  data[,4] = as.double(as.character(data[,4]))
  #data[,5] = as.double(as.character(data[,5])) #使用默认应变
  data[,5] = as.double(as.character(data[,2]))/l_0*100 #使用位移应变
  ...
  library(cubature)#积分函数library
  
  data_integrate[1,5] = cubintegrate(f, data[1,5], data[data_index[1,1],5],method = "pcubature")[1] #线性插值函数面积积分：加载功
  if(is.na(data_integrate[1,5])){
    data_integrate[1,5] = cubintegrate(f, data[1,5], data[data_index[1,1],5],method = "hcubature")[1] #线性插值函数面积积分：加载功
  }
  if(num_cycle>1){
    for (i in 1:(num_cycle-1)) {
      f <- approxfun(x = data[(data_index[i,3]:data_index[i+1,1]),5], y = data[(data_index[i,3]:data_index[i+1,1]),4],
                     method = "linear")#线性插值函数
      data_integrate[i+1,5] = cubintegrate(f, data[data_index[i,3],5], data[data_index[i+1,1],5],method = "pcubature")[1] #线性插值函数面积积分：加载功
      
      if(is.na(data_integrate[i+1,5])){
        data_integrate[i+1,5] = cubintegrate(f, data[data_index[i,3],5], data[data_index[i+1,1],5],method = "hcubature")[1] #线性插值函数面积积分：加载功
  ...
  for (i in 1:num_cycle) {
    data_integrate[i,6] = data_integrate[i,5] - data_integrate[i,2]#计算内耗
  }
  
  
  
  
  names(data_integrate)[1:6] = c("Cycle", "Work per Unit Volume","超弹性应变(仅卸载过程)",
                                 "超弹性应变(最后一个循环按曲线结束点应变计算)","加载功","内耗")
  #*********************曲线数据***********************
